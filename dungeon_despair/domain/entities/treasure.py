...
class Treasure(Entity):
	...
	trapped_chance: float = Field(..., description="The chance that this treasure' trap gets triggered", required=True)
	dmg: float = Field(..., description="The damage this treasure deals to a hero if the internal trap is triggered", required=True)
	
	modifier: Modifier = Field(None, description="The modifier the treasure could apply to the hero if the internal trap is triggered",
	                           required=False)
