...
def check_level_playability(level: Level, condition: ScenarioType) -> bool:
    if condition == ScenarioType.EXPLORE:
        # Current room must always be a room

        assert (
            level.current_room in level.rooms.keys()
        ), f"Current room must be a room and not a corridor (currently: {level.current_room})."
        # Explore mission require at least three rooms

        assert (
            len(level.rooms) >= 3
        ), f"Explore missions should have at least 3 rooms; found: {len(level.rooms)}."
        ...
        for room in level.rooms.values():
            for enemy in room.encounter.entities["enemy"]:
                assert (
                    len(enemy.attacks) > 0
                ), f"Enemies must all have at least one attack: {enemy.name} in {room.name} has {len(enemy.attacks)} attacks."
        ...
        raise NotImplementedError(
            "Treasure Hunts scenarios have not been implemented yet!"
        )
