...
class Level(BaseModel):
    ...
    level_name: str = Field(
        default="Unnamed Level", description="The name of the level.", required=True
    )

    rooms: Dict[str, Room] = Field(
        default={}, description="The rooms in the level.", required=True
    )
    corridors: Dict[str, Corridor] = Field(
        default={}, description="The corridors in the level.", required=True
    )
    connections: Dict[str, Dict[Direction, str]] = Field(
        default={}, description="The connections in the level.", required=True
    )

    current_room: str = Field(
        default="", description="The currently selected room.", required=True
    )

    @property
    def cost(self) -> float:
        # The cost of the level is the sum of the costs of all rooms and corridors

        return sum(room.cost for room in self.rooms.values()) + sum(
            corridor.cost for corridor in self.corridors.values()
        )

    def save_to_file(self, filename: str, conversation: str) -> None:
        all_images = os.listdir(config.temp_dir)
        images = {
            image_path: PIL.Image.open(os.path.join(config.temp_dir, image_path))
            for image_path in all_images
        }
        bin_data = {"level": self, "images": images, "conversation": conversation}
    ...
    def export_level_as_scenario(self, filename: str) -> None:
        # get all sprites
            ...
            for encounter in corridor.encounters:
                for entity_type in encounter.entities.keys():
                    for entity in encounter.entities[entity_type]:
                        all_sprites.append(os.path.basename(entity.sprite))
        sprites = {
            sprite_path: PIL.Image.open(os.path.join(config.temp_dir, sprite_path))
    ...
    def get_level_subset(
        self, corridor: Corridor, opposite_direction: bool = False
    ) -> Tuple[List[Room], List[Corridor]]:
        # This method assumes that the corridor is NOT part of a loop

        rooms, corridors = [], []
        to_expand = (
            [corridor.room_to] if not opposite_direction else [corridor.room_from]
        )
                ...
                if area in self.rooms.keys():
                    ...
                    if opposite_direction:
                        to_expand.extend(
                            [
                                c.name
                                for c in self.get_corridors_by_room(area)
                                if c.room_from != area and c.name not in corridors
                            ]
                        )
                    else:
                        to_expand.extend(
                            [
                                c.name
                                for c in self.get_corridors_by_room(area)
                                if c.room_to != area and c.name not in corridors
                            ]
                        )
                else:
    ...
    def remove_hanging_rooms(self, ref_room: Optional[str] = None) -> None:
