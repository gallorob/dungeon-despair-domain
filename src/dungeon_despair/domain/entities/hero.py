...
class Hero(Entity):
    ...
    trap_resist: float = Field(
        ..., description="The chance this hero will not trigger traps", required=True
    )
    stress_resist: float = Field(
        ...,
        description="The percentage resistance of the hero to stress",
        required=True,
    )

    modifiers: List[Modifier] = Field(
        [], description="The hero's modifiers", required=True
    )
